...
def parse_invoice_number(invoice_number: str) -> tuple[int, ...]:
    ...
    """
    return tuple(int(item) for item in invoice_number.split("-"))
...
def next_invoice_number(invoice_number: str) -> str:
    ...
    """
    y, d = parse_invoice_number(invoice_number)
    d = d + 1 if y != int(invoice_number[0]) else 1

    new_y = str(y)
    new_d = f"{d:04}"  # Formats d as a zero-padded 4-digit string

    return f"{new_y}-{new_d}"
